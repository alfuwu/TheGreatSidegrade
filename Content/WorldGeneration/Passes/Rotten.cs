...
public class Rotten {
                ...
                if (num822 == num816 || num821 == 0) {
                    for (int num823 = (int) GenVars.worldSurfaceLow; num823 < Main.worldSurface - 1; num823++) {
                        if (Main.tile[num822, num823].HasTile || Main.tile[num822, num823].WallType > 0) {
                            if (num822 == num816) {
                                num821 = 20;
                                RottenRunner(num822, num823, WorldGen.genRand.Next(150) + 150, makeOrb: true);
                            } else if (WorldGen.genRand.NextBool(35) && num821 == 0) {
                                num821 = 30;
                                bool makeOrb = true;
                                RottenRunner(num822, num823, WorldGen.genRand.Next(50) + 50, makeOrb);
                            }
            ...
            for (int num828 = num817; num828 < num818; num828++) {
                ...
                for (int num829 = (int) GenVars.worldSurfaceLow; num829 < num827; num829++) {
                    if (Main.tile[num828, num829].HasTile) {
                        ...
                        //    Main.tile[num828, num829].WallType = (ushort) ModContent.WallType<HardendRotsandWall>();
                        //else if (Main.tile[num828, num829].WallType == WallID.Sandstone)
                        //    Main.tile[num828, num829].WallType = (ushort) ModContent.WallType<RotsandstoneWall>();
    ...
    public static void RottenRunner(int i, int j, int steps, bool makeOrb = false) {
        bool flag = false;
        bool flag2 = false;
        bool flag3 = false;
        if (!makeOrb)
